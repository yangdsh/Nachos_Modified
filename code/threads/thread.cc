...
void TS() {
	...
	for (int i = 1; i <= 128; ++i) {
		if (freeTid[i] == 0) {
			printf("uid: %d\ttid: %d\tname: %s\tstatus:%d\n", Tpool[i]->getUid(), Tpool[i]->getTid(), Tpool[i]->getName(), Tpool[i]->getStatus());
		}
	}
}

Thread* createThread(char* threadName, int prior = 0)
{
	Thread* newThread;
	if (tNum < 128)
	{
		newThread = new Thread(threadName, prior);
	}
	else newThread = NULL;
	return newThread;
}
...
Thread::Fork(VoidFunctionPtr func, int arg)
{
    ...
    scheduler->JustCreated(this);	// ReadyToRun assumes that interrupts 
					// are disabled!
...
Thread::Yield ()
{
    if(iftime)
    {
		if(UsedTime < TimeTick) TimeTick = TimeTick - UsedTime ;
    	else
    	{
    		//反馈队列 
			prior ++ ;
