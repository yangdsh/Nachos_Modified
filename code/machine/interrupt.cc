...
Interrupt::SetLevel(IntStatus now)
{
    ...
    ASSERT((now == IntOff) || (inHandler == FALSE) || allowed);// interrupt handlers are 
...
Interrupt::Schedule(VoidFunctionPtr handler, int arg, int fromNow, IntType type)
{
	...
	//printf("Scheduling interrupt handler the %s at time = %d\n", 
	//				intTypeNames[type], when);
    ASSERT(fromNow > 0);
...
PrintPending(int arg)
{
    ...
    if(time) printf("Interrupt handler %s, scheduled at %d\n", 
